...
const AuthorRequests = {
  ...
  post: (url: string, body: Author) => axiosInstance.post<Author>(url, body).then(responseBody),
  patch: (url: string, body: Author) => axiosInstance.patch<Author>(url, body).then(responseBody),
...
export const AuthorsClient = {
  ...
  findAuthorById: (id: number): Promise<Author> => AuthorRequests.get(`${AUTHORS_URL_API}/${id}`),
  updateAuthor: (id: number, body: Author): Promise<Author> =>
    AuthorRequests.patch(`${AUTHORS_URL_API}/${id}`, body),
  removeAuthor: (id: number): Promise<Author> => AuthorRequests.delete(`${AUTHORS_URL_API}/${id}`),
};
