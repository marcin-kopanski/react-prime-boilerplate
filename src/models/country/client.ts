...
const CountryRequests = {
  ...
  post: (url: string, body: Country) => axiosInstance.post<Country>(url, body).then(responseBody),
  patch: (url: string, body: Country) => axiosInstance.patch<Country>(url, body).then(responseBody),
  delete: (url: string) => axiosInstance.delete<Country>(url).then(responseBody),
};

export const CountriesClient = {
  createCountry: (body: Country) => CountryRequests.post(COUNTRIES_URL_API, body),
  findAllCountries: (): Promise<Country[]> => CountryRequests.get(COUNTRIES_URL_API),
